...
describe("Matrix operations", () => {
	...
	describe("add", () => {
		it ("adds two matrices", () => {
			const A = new GenericMatrix<number>(2,3).fill(1);
			const B = new GenericMatrix<number>(2,3).fill(2);
			const C = new GenericMatrix<number>(2,3).fill(3);
			const R = add(A, B);
			assert.isTrue(R.equals(C));
		});
		it ("throws an error if matrices are invalidly size", () => {
			const A = new GenericMatrix<number>(2,3).fill(0);
			const B = new GenericMatrix<number>(0,3).fill(0);
			const C = new GenericMatrix<number>(2,0).fill(0);
	...
	describe("subtract", () => {
		it ("subracts two matrices", () => {
			const A = new GenericMatrix<number>(2,3).fill(3);
			const B = new GenericMatrix<number>(2,3).fill(2);
			const C = new GenericMatrix<number>(2,3).fill(1);
